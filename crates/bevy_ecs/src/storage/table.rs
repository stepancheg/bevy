...
pub struct Column {
    data: BlobVec,
    component_ticks: Vec<UnsafeCell<ComponentTicks>>,
}
...
impl Column {
    ...
    pub(crate) unsafe fn swap_remove_unchecked(&mut self, row: TableRow) {
        ...
        self.component_ticks.swap_remove(row.as_usize());
    }
    ...
    pub(crate) fn swap_remove_and_forget(
        ...
        (row.as_usize() < self.data.len()).then(|| {
            ...
            let component_ticks = self
                .component_ticks
                .swap_remove(row.as_usize())
                .into_inner();
            (data, component_ticks)
        })
    }
    ...
    pub(crate) unsafe fn initialize_from_unchecked(
        ...
        *self.component_ticks.get_unchecked_mut(dst_row.as_usize()) =
            other.component_ticks.swap_remove(src_row.as_usize());
    }
    ...
    pub(crate) unsafe fn push(&mut self, ptr: OwningPtr<'_>, ticks: ComponentTicks) {
        self.data.push(ptr);
        self.component_ticks.push(UnsafeCell::new(ticks));
    }

    #[inline]
    pub(crate) fn reserve_exact(&mut self, additional: usize) {
        self.data.reserve_exact(additional);
        self.component_ticks.reserve_exact(additional);
    }
    ...
    /// Fetches the slice to the [`Column`]'s "added" and "changed" detection ticks.
    ///
    /// Note: The values stored within are [`UnsafeCell`].
    /// Users of this API must ensure that accesses to each individual element
    /// adhere to the safety invariants of [`UnsafeCell`].
    #[inline]
    pub fn get_component_ticks_slice(&self) -> &[UnsafeCell<ComponentTicks>] {
        &self.component_ticks
    }
        ...
        (row.as_usize() < self.data.len())
            ...
            .then(|| unsafe {
                (
                    ...
                    TickCells {
                        component_ticks: self.component_ticks.get_unchecked(row.as_usize()),
                    },
                )
            })
    }
    ...
    /// Fetches the "added" and "changed" detection tick for the value at `row`.
    ///
    /// Returns `None` if `row` is out of bounds.
    ///
    /// Note: The values stored within are [`UnsafeCell`].
    ...
    /// adhere to the safety invariants of [`UnsafeCell`].
    #[inline]
    pub fn get_component_ticks(&self, row: TableRow) -> Option<&UnsafeCell<ComponentTicks>> {
        self.component_ticks.get(row.as_usize())
    }
    ...
    /// Fetches the "added" and "change" detection tick for the value at `row`.
    /// Unlike [`Column::get_component_ticks`] this function does not do any bounds checking.
    ///
    /// # Safety
    /// `row` must be within the range `[0, self.len())`.
    #[inline]
    pub unsafe fn get_component_ticks_unchecked(
        &self,
        row: TableRow,
    ) -> &UnsafeCell<ComponentTicks> {
        debug_assert!(row.as_usize() < self.component_ticks.len());
        self.component_ticks.get_unchecked(row.as_usize())
    }
    ...
    pub unsafe fn get_ticks_unchecked(&self, row: TableRow) -> ComponentTicks {
        debug_assert!(row.as_usize() < self.component_ticks.len());
        self.component_ticks.get_unchecked(row.as_usize()).read()
    }
    ...
    pub(crate) fn check_change_ticks(&mut self, change_tick: Tick) {
        for component_ticks in &mut self.component_ticks {
            component_ticks.get_mut().added.check_tick(change_tick);
            component_ticks.get_mut().changed.check_tick(change_tick);
        }
    }
}
...
impl Table {
    ...
    pub(crate) unsafe fn allocate(&mut self, entity: Entity) -> TableRow {
        ...
        for column in self.columns.values_mut() {
            ...
            column.component_ticks.push(UnsafeCell::new(ComponentTicks {
                added: Tick::new(0),
                changed: Tick::new(0),
            }));
        }
